...
global $connection;
	...
	while ($row = $result->fetch_row()) {
		if(preg_match("#http://akas.imdb.com/(name|title)/(nm|tt)([0-9]+)/?#", $row[0], $opdel)) {
		    ...
		    $imdb_link++;
                } else if(preg_match("#https://www.danskefilm.dk/(film|skuespiller|tvserie|stumfilm|julekalender|tegnefilm).php\?id=([0-9]*)#", $row[0], $opdel)) {
                        $sum_links .= " - <a href=\"" . $row[0] . "\" class=\"danskefilmlink\" >danskefilm</a>";
                } else if(preg_match("#https?://danskfilmogtv.dk/content.php\?page=(media|persons|main)(&sort=lastname)?&value=([0-9]*)#", $row[0], $opdel)) {
